...
const BASKET_VAULT_ID = "BASKETvau1tXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX";
const SSS_PROGRAM_ID  = "SSSxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx";
const SVS1_PROGRAM_ID = "SVS1VauLt1111111111111111111111111111111111";

// BASKET mint created by SSS deploy — copy here
const BASKET_MINT     = "BASKETmint111111111111111111111111111111111";

// Asset mints (devnet — create test mints with spl-token create-token)
const ASSET_MINTS = {
  PAXG: "PAXGmint1111111111111111111111111111111111",
  WTI:  "WTImint11111111111111111111111111111111111",
  BTC:  "WBTCmint1111111111111111111111111111111111",
  XAG:  "XAGmint11111111111111111111111111111111111",
  DXY:  "DXYmint111111111111111111111111111111111111",
  RWA:  "RWAmint111111111111111111111111111111111111",
};

// Switchboard aggregator stubs (devnet)
const SB_STUB = "SW1TCHBOARDxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx";
...
async function main() {
  ...
  const program = new anchor.Program(idl, BASKET_VAULT_ID, provider);

  // ── Derive PDAs ─────────────────────────────────────────────────────────
  const [globalConfig] = anchor.web3.PublicKey.findProgramAddressSync(
    [Buffer.from("global_config")],
    new PublicKey(BASKET_VAULT_ID)
  );
  ...
  const assetRegistry = Object.entries(ASSET_MINTS).map(([, mintStr], i) => ({
    ...
    switchboardAggregator: new PublicKey(SB_STUB),
  ...
  try {
    const tx = await (program.methods as any)
      .initialize({
        sssProgram:         new PublicKey(SSS_PROGRAM_ID),
        svsProgramId:       new PublicKey(SVS1_PROGRAM_ID),
      ...
      .accounts({
        ...
        basketMint:    new PublicKey(BASKET_MINT),
        globalConfig,
        vaultAuthority,
