...
import { getAdaptiveCR } from "./utils/basket-sdk";
...
function InnerApp() {
  ...
  async function doAction() {
    ...
    setTxStatus("pending");
    setTxMsg(tab === "deposit" ? "Sending to SVS-1 vault…"
           : tab === "mint"    ? "Verifying CR → minting…"
           :                     "Burning BASKET → redeeming…");

    // Simulate tx flow (replace with real SDK calls)
    await new Promise(r => setTimeout(r, 1800));
    setTxStatus("success");
    setTxMsg(`✓ ${tab === "deposit" ? "Deposited" : tab === "mint" ? "Minted" : "Redeemed"} ${amount} ${tab === "mint" || tab === "redeem" ? "BASKET" : selectedAsset}`);
    setAmount("");
    refresh();
  }
      ...
      <div style={S.grid}>
        ...
        <div style={S.leftPanel}>
          ...
          <div style={S.crCard}>
            ...
            <div style={S.crRow}>
              <div>
                <div style={S.crLabel}>Current CR</div>
                <div style={{ ...S.crValue, color: crColor }}>
                  {state ? Math.round((state.totalMinted > 0 ? 217 : 0)) : "—"}
            ...
            <div style={S.barTrack}>
              <div style={{ ...S.barFill, width: "60%", background: crColor }} />
            </div>
          ...
          <div style={S.statsRow}>
            {[
              { label: "TVL",        value: "$0",         sub: "collateral"   },
              { label: "Circulating", value: state ? state.totalMinted.toFixed(2) : "0", sub: "BASKET"  },
              { label: "Ins. Fund",  value: state ? `$${state.insuranceFund.toFixed(2)}` : "$0", sub: "0.1% fee" },
            ].map(s => (
