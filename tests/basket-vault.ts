...
describe("basket-vault", () => {
  ...
  it("toggles emergency mode", async () => {
    ...
    let config = await (program.account as any).globalConfig.fetch(globalConfig);
  ...
  // ── 7. Liquidation + insurance math ─────────────────────────────────────

  it("validates liquidation threshold and insurance absorption", () => {
    const LIQ_THRESHOLD_BPS = 12_000; // 120%
    const KEEPER_BONUS_BPS = 500;     // 5%
    ...
    function simulate(
      ...
      const crBeforeBps = Math.floor((collateralValue * 10_000) / debtBefore);
      const liquidatable = crBeforeBps < LIQ_THRESHOLD_BPS;
      ...
      const requestedBonus = Math.floor((repayAmount * KEEPER_BONUS_BPS) / 10_000);
      const keeperBonus = Math.min(requestedBonus, insuranceBefore);
      const debtAfterRepay = Math.max(0, debtBefore - repayAmount - keeperBonus);
      let insuranceAfter = insuranceBefore - keeperBonus;

      const badDebt = Math.max(0, debtAfterRepay - collateralValue);
      const insuranceCover = Math.min(insuranceAfter, badDebt);
      insuranceAfter -= insuranceCover;
      const debtAfter = debtAfterRepay - insuranceCover;
      const emergency = badDebt > insuranceCover;
    ...
    // Healthy position should not liquidate
    const healthy = simulate(15000, 10000, 200, 1000);
    expect(healthy.liquidatable).to.eq(false);
    expect(healthy.debtAfter).to.eq(10000);

    // Underwater position: debt 10k, collateral 8k, insurance 500, repay 2k
    // bonus=100, debtAfterRepay=7900, badDebt=0 => no emergency
    const repaired = simulate(8000, 10000, 500, 2000);
    expect(repaired.liquidatable).to.eq(true);
    expect(repaired.keeperBonus).to.eq(100);
    expect(repaired.debtAfter).to.eq(7900);
    expect(repaired.emergency).to.eq(false);

    // Deeply insolvent with low insurance should trigger emergency
    const insolvent = simulate(4000, 12000, 100, 1000);
    expect(insolvent.liquidatable).to.eq(true);
    expect(insolvent.emergency).to.eq(true);

    console.log("✓ Liquidation + insurance accounting: passed");
  });
});
